...
class Doodler:
    ...
    def __init__(self, rows=10, cols=10, bkg=0, fog=1, nbias=0.0, wrange=[0.1, 0.6], hrange=[0.1, 0.6],
        ...
        self.max_width = cols - 1;
        self.max_height = rows - 1
        self.width_range = wrange;
        self.height_range = hrange  # Ranges for height and width of doodles
        self.background = bkg
        self.foreground = fog
    ...
    def cover_cell(self, r, c):
        self.cover_map[r, c] = 1

    def uncover_cell(self, r, c):
        self.cover_map[r, c] = 0

    def cell_covered(self, r, c):
        return self.cover_map[r, c] == 1
    ...
    def check_availability(self, center, half_width, half_height):
        hw = half_width + self.figure_gap;
        hh = half_height + self.figure_gap
    ...
    def gen_image_params(self, wrange=[0, 1], hrange=[0, 1]):
        ...
        while not (image_fits):
            w = self.gen_random_width(*wrange);
            h = self.gen_random_height(*hrange)
            c = self.canvas_center if (self.centered and not (self.multi)) else self.gen_random_center(w, h)
            w2 = max(1, math.floor(w / 2));
            h2 = max(1, math.floor(h / 2))
    ...
    def gen_bar(self, center, w2, h2):
        ...
        c1 = round(center[1] - w2);
        c2 = round(center[1] + w2)
        r1 = round(center[0] + factor * h2);
        r2 = round(center[0] - factor * h2)
    ...
    def gen_bar_predicate(self, r1, c1, r2, c2):
        ...
        def fp_horizontal(r, c):
            return abs(r - r1) <= self.bar_margin and min(c1, c2) <= c <= max(c1, c2)
        ...
        if r1 == r2:
            return fp_horizontal
        elif c1 == c2:
            return fp_vertical
        else:
            p1 = self.get_cell_center(r1, c1);
            p2 = self.get_cell_center(r2, c2)  # real-valued coords (x,y)
            dx = p2[0] - p1[0]
            ...
            a = slope;
            b = -1;
            d = p1[1] - slope * p1[0]  # params for the shortest-dist to point equation.
    ...
    def gen_random_polygon_size(self):
        return random.choice(self.polygon_sizes)

    def get_polygon_index(self, size):
        return self.polygon_sizes.index(size)
    ...
    def gen_polygon(self, center, w2, h2, n=5, regular=False):
        ...
        pts = gen_convex_polygon(n, xr=(0, 1), yr=(0, 1), plot=False, vect=False,
                                 regular=regular)  # all coords in range [0,1]
        rmin = center[0] - h2;
        cmin = center[1] - w2;
        dr = 2 * h2 + 1;
        dc = 2 * w2 + 1
    ...
    def gen_image(self, types=['ball'], wr=None, hr=None, figcount=1, fillfrac=None, poly=None):
        ...
        wrange = wr if wr != None else self.width_range;
        hrange = hr if hr != None else self.height_range
    ...
    def gen_label(self, all_image_types, curr_image_types, figcount=1,
        ...
        if poly is not None:
            return self.get_polygon_index(poly)  # Polygon testing.
    ...
    def gen_random_cases(self, count, image_types=_doodle_image_types_, flat=True, wr=None, hr=None,
        ...
        if fillfrac is not None:
            ...
            if type(fillfrac) == tuple:
                frange = fillfrac
            elif fillfrac == 'all':
                frange = (1.0, 1.0)
            else:
                frange = (fillfrac, fillfrac)
    ...
    def gen_random_mono_shape_cases(self, count, image_types=_doodle_image_types_, figcount=(1, 5),
        ...
        for c in range(count):
            im = random.choice(image_types);
            fc = random.randint(*figcount)
            lab = self.calc_mono_shape_label(image_types, im, figcount, fc)
            cases.append(
                self.gen_random_case(image_types=[im], flat=flat, wr=wr, hr=hr, figcount=fc, tlen=tlen, label=lab))
        return cases
...
def gen_standard_cases(count=10, rows=50, cols=50, types=_doodle_image_types_, wr=[0.2, 0.5], hr=[0.2, 0.4],
                       ...
                       fc=(1, 1), auto=False, mono=True, one_hots=True, multi=False, random_seed=None):
    return gen_doodle_cases(count, rows=rows, cols=cols, imt=types, wr=wr, hr=hr,
                            nbias=noise, cent=cent, show=show, flat=flat,
                            mono=mono, auto=auto, fc=fc, one_hots=one_hots, multi=multi, random_seed=random_seed)
...
def gen_doodle_cases(count, rows=30, cols=30, imt=None, hr=[0.15, 0.3], wr=[0.15, 0.3],
                     ...
                     gap=1, multi=False, mono=False, dbias=0.7, poly=(4, 5, 6), random_seed=None):
...
def gen_convex_polygon(n, xr=(0, 1), yr=(0, 1), plot=True, vect=False, regular=False):
    ...
    if regular:
        ...
        xgroups = group(xvals[1:-1], groupsize);
        ygroups = group(yvals[1:-1], groupsize)
    else:
        # Generate n random x and y values; sort them.
        xvals = sorted(np.random.uniform(0, 1, size=n));
        yvals = sorted(np.random.uniform(0, 1, size=n))
        # Randomly divide the non-extrema into two subsets.
        xgroups = random_group(xvals[1:-1], 2);
        ygroups = random_group(yvals[1:-1], 2)
    ...
    xgroups = [[xmin] + sorted(g) + [xmax] for g in xgroups];
    ygroups = [[ymin] + sorted(g) + [ymax] for g in ygroups]
    ...
    if plot or not (vect):
        pt = (0.0, 0.0);
        pts = []  # If the algorithm works properly, the first pt gets added at the end of the cycle.
        for v in vectors:
...
def graphics_end_interactive_mode():
    PLT.show(block=True)
    PLT.ioff()
